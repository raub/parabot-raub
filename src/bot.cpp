...
void BotCreate(int fixedPersNr) {
	...
	if (
		mod_id == VALVE_DLL || mod_id == AG_DLL || mod_id == HUNGER_DLL ||
		mod_id == DMC_DLL || mod_id == GEARBOX_DLL
	) { // set colors
	...
	if (mod_id == VALVE_DLL || mod_id == AG_DLL || mod_id == HUNGER_DLL || mod_id == GEARBOX_DLL) {
		pBot->parabot->goalFinder.addGoal(
			G_MOVE | G_VIEW, GOAL_UNCONDITIONAL, goalLayTripmine, weightLayTripmine
		);
	} else if (mod_id == HOLYWARS_DLL) {
		pBot->parabot->goalFinder.addGoal(
			G_MOVE, GOAL_UNCONDITIONAL, goalWaitAtNavpoint, weightWaitForHalo
		);
	}
	
	pBot->parabot->goalFinder.addGoal(G_VIEW, PI_DAMAGE, goalLookAround, weightLookAroundDamage);
	pBot->parabot->goalFinder.addGoal(G_ACTION, PI_DAMAGE, goalBunnyHop, weightBunnyHop);

	pBot->parabot->goalFinder.addGoal(
		G_VIEW, PI_PLAYER, goalReactToUnidentified, weightReactToUnidentified
	);
	
	pBot->parabot->goalFinder.addGoal(G_VIEW, PI_NEWAREA, goalLookAtNewArea, weightLookAtNewArea);

	pBot->parabot->goalFinder.addGoal(G_MOVE, PI_FRIEND, goalMakeRoom, weightMakeRoom);

	if (
		(mod_id == CSTRIKE_DLL) || (mod_id == TFC_DLL) ||
		gearbox_ctf || (mod_id == AG_DLL && !strcmp(ag_gamemode, "ctf"))
	) {
		pBot->parabot->goalFinder.addGoal(G_ACTION, PI_FRIEND, goalAssistFire, weightAssistFire);
		pBot->parabot->goalFinder.addGoal(G_MOVE, PI_FRIEND, goalFollow, weightFollowLeader);
	}

	pBot->parabot->goalFinder.addGoal(G_VIEW, PI_FOE, goalLookAround, weightLookAroundPlayerSound);
	pBot->parabot->goalFinder.addGoal(G_ACTION, PI_FOE, goalArmBestWeapon, weightArmBestWeapon);
	pBot->parabot->goalFinder.addGoal(G_VIEW, PI_FOE, goalShootAtEnemy, weightShootAtEnemy);
	pBot->parabot->goalFinder.addGoal(G_MOVE | G_VIEW, PI_FOE, goalCloseCombat, weightCloseCombat);
	pBot->parabot->goalFinder.addGoal(G_MOVE | G_VIEW, PI_FOE, goalSilentAttack, weightSilentAttack);
	pBot->parabot->goalFinder.addGoal(G_MOVE | G_VIEW, PI_FOE, goalUseTank, weightUseTank);
//	pBot->parabot->goalFinder.addGoal(G_MOVE | G_VIEW, PI_FOE, goalRangeAttack, weightRangeAttack);
//	pBot->parabot->goalFinder.addGoal(G_MOVE | G_VIEW, PI_FOE, goalPrepareAmbush, weightPrepareAmbush);
	pBot->parabot->goalFinder.addGoal(G_MOVE, PI_FOE, goalHuntEnemy, weightHuntEnemy);
	pBot->parabot->goalFinder.addGoal(G_MOVE, PI_FOE, goalFleeEnemy, weightFleeEnemy);
	pBot->parabot->goalFinder.addGoal(G_MOVE, PI_FOE, goalTakeCover, weightTakeCover);
...
void BotStartGame(bot_t *pBot) {
	...
	if (mod_id == TFC_DLL) {
		...
		// counter terrorist
		if (pBot->start_action == MSG_CS_CT_SELECT) {
			pBot->start_action = MSG_CS_IDLE; // switch back to idle
		...
		// terrorist select
		if (pBot->start_action == MSG_CS_T_SELECT) {
			pBot->start_action = MSG_CS_IDLE; // switch back to idle
...
bool BotEntityIsVisible(bot_t *pBot, Vector dest) {
	...
	UTIL_TraceLine(
		pBot->pEdict->v.origin + pBot->pEdict->v.view_ofs, dest,
		ignore_monsters, pBot->pEdict->v.pContainingEntity, &tr
	);
	
	// check if line of sight to object is not blocked (i.e. visible)
	return tr.flFraction >= 1.0 ? TRUE : FALSE;
}
...
// TheFatal - START from Advanced Bot Framework (Thanks Rich!)
	...
	if (pBot->not_started) {
		...
		g_engfuncs.pfnRunPlayerMove(
			pEdict, pEdict->v.v_angle, 0.0, 0, 0, pEdict->v.button, 0, pBot->msecval
		);
		pEdict->v.flags |= FL_FAKECLIENT;
		return;
	}
	...
	// don't call this before round has
	if ((worldTime() > roundStartTime) && !pb_pause) {
		pBot->parabot->botThink(); // started
	} else { // prevent crashes...???
		g_engfuncs.pfnRunPlayerMove(pEdict, Vector(0, 0, 0), 0, 0, 0, 0, 0, pBot->msecval);
	}
