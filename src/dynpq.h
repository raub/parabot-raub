...
template <class key_type>
dynamic_priority_queue<key_type>::dynamic_priority_queue(std::vector<key_type>& v)
: Indices(v.size()), c(v.size()), first(&(*v.begin())), csize(v.size()) {
	index_type i;
...
template <class key_type>
void dynamic_priority_queue<key_type>::changeKeyAt(index_type at, key_type k) {
	index_type idx = Indices[at];
	assert(idx < csize); // value still present in the queue?
	
	// in case of equality, do nothing
	if (*c[idx] != k) {
		if (comp(&k, c[idx])) {
