...
float weightReactToUnidentified(CParabot *pb, PB_Percept*item) {
	...
	if (time < 2) {
		weight = 2 - time; // look 2 seconds at new sounds
	}
...
void goalLookAtNewArea(CParabot *pb, PB_Percept*item) {
	...
	int cycle = static_cast<int>((worldTime() - startLook[b]) * 128);
	cycle &= 255;
...
float weightLookAtNewArea(CParabot *pb, PB_Percept*item) {
	...
	if (time < 2) {
		weight = 2 - time; // look 2 seconds at new areas
	}
...
void goalLookAround(CParabot *pb, PB_Percept*item) {
	...
	if (item) {
		switch(item->pClass) {
		case PI_LASERDOT:
			pb->setGoalViewDescr("LookAround (Laserdot)");
			break;
		case PI_DAMAGE:
			pb->setGoalViewDescr("LookAround (Damage)");
			break;
		case PI_EXPLOSIVE:
			pb->setGoalViewDescr("LookAround (ExplosiveSound)");
			break;
		case PI_FOE:
			pb->setGoalViewDescr("LookAround (PlayerSound)");
			break;
		case PI_NEWAREA:
			pb->setGoalViewDescr("LookAround (NewArea)");
			break;
...
float weightAssistFire(CParabot *pb, PB_Percept*item) {
	...
	if (!pb->combat.weapon.bestWeaponUsable()) {
		return 0;
	}
	if (item->entity->v.button & IN_ATTACK) {
		return 2;
	} else {
		return 0;
	}
...
float weightCollectItems(CParabot *pb, PB_Percept*item) {
	...
	if (pb->actualNavpoint) { // when getting to a navpoint is all we want...
		if (pb->actualNavpoint->offersHealth() && (pb->needs.needForHealth() > 0)) {
			wish = 0;
		} else if (pb->actualNavpoint->offersArmor() && (pb->needs.needForArmor() > 0)) {
			wish = 0;
		} else if (pb->actualNavpoint->offersCamping() && (pb->needs.desireFor(NAV_S_CAMPING) == wish)) {
			wish = 0;
		} else if (pb->actualNavpoint->type()==NAV_F_TANKCONTROLS && (pb->needs.desireFor(NAV_F_TANKCONTROLS) == wish)) {
			wish = 0;
		} else if (pb->actualNavpoint->type()==NAV_S_USE_TRIPMINE && (pb->needs.desireFor(NAV_S_USE_TRIPMINE) > 0)) {
			wish /= 2;
		}
	}
...
void goalGetItem(CParabot *pb, PB_Percept*item) {
	...
	if (pb->action.gotStuck()) {
		item->flags |= PI_UNREACHABLE;
	}
	
	switch(item->pClass) {
	case PI_WEAPONBOX:
		pb->setGoalMoveDescr("GetItem (Weaponbox)");
		break;
	case PI_HALO:
		pb->setGoalMoveDescr("GetItem (Halo)");
		break;
	default:
		pb->setGoalMoveDescr("GetItem (?)");
		break;
	}
}
...
void goalGetAway(CParabot *pb, PB_Percept*item) {
	...
	switch(item->pClass) {
	case PI_LASERDOT:
		pb->setGoalMoveDescr("GetAway (Laserdot)");
		break;
	case PI_EXPLOSIVE:
		pb->setGoalMoveDescr("GetAway (Explosive)");
		break;
	case PI_FOE:
		pb->setGoalMoveDescr("GetAway (Enemy)");
		break;
	default:
		pb->setGoalMoveDescr("GetAway (?)");
		break;
	}
}
...
float weightGetAwayEnemy(CParabot *pb, PB_Percept*item) {
	assert(item != 0);
	if (item->isTrackable()) {
		return (4 - item->rating - pb->needs.wishForCombat());
	}
...
float weightLoadHealthOrArmor(CParabot *pb, PB_Percept*item) {
	...
	if (pb->actualNavpoint && !pb->senses.underFire()) {
		if (pb->actualNavpoint->offersHealth()) {
			weight = pb->needs.needForHealth();
		}
		if (pb->actualNavpoint->offersArmor()) {
			weight = pb->needs.needForArmor();
		}
...
float weightLayTripmine(CParabot *pb, PB_Percept*item) {
	if (!pb->combat.weapon.available(VALVE_WEAPON_TRIPMINE)) {
		return 0;
	}
	...
	if (pb->actualNavpoint && !pb->senses.underFire()) {
		...
		if (mine) {
			nextMine = (mine->v.origin - pb->actualNavpoint->pos()).Length();
		}
...
float weightCamp(CParabot *pb, PB_Percept*item) {
	...
	if (pb->actualNavpoint && !pb->senses.underFire()) {
		if (pb->actualNavpoint->offersCamping() && pb->combat.weapon.bestWeaponUsable()) {
...
void goalUseTank(CParabot *pb, PB_Percept*item) {
	...
	if ((lastCall[b]+0.5) < worldTime() || (pb->campTime > 0 && pb->ent->v.viewmodel)) { // in first call press button
		pb->campTime = -2;
...
float weightUseTank(CParabot *pb, PB_Percept*item) {
	...
	if (pb->actualNavpoint && !pb->senses.underFire()) {
		if (pb->actualNavpoint->type() == NAV_F_TANKCONTROLS) {
			...
			if (item && item->isVisible() && item->distance > 300) {
				weight += 10;
			}
		}
	}
...
float weightFollowLeader(CParabot *pb, PB_Percept*item) {
	...
	if (
		(item->isVisible()) && // leader visible
		(item->lastSeenPos.z < (pb->botPos().z + 16)) && // not higher than bot
		((item->lastSeenPos - pb->botPos()).Length() < 500)
	) { // not too far away
		return 2;
	}
	return 0;
...
float weightFollowEnemy(CParabot *pb, PB_Percept*item) {
	...
	if (item->pState & PI_TACTILE) {
		return (5 + item->rating + pb->needs.wishForCombat());
	} else if (item->rating < -3) {
		return 0; // too bad
	} else {
		return (item->rating + pb->needs.wishForCombat());
	}
}
...
void goalMakeRoom(CParabot *pb, PB_Percept*item) {
	...
	if ((lastCall[b]+0.5) < worldTime()) { // in first call init newWp
		Vector forward;
		if (pb->actualPath) {
			forward = (pb->waypoint.pos(pb->ent) - pb->botPos());
		} else {
			forward = pb->action.getMoveDir();
		}
		forward.Normalize();
...
float weightMakeRoom(CParabot *pb, PB_Percept*item) {
	...
	float newDist = (
		(pb->botPos() + pb->ent->v.velocity / 20) -
		(item->entity->v.origin + item->entity->v.velocity)
	).Length();
