...
float weightHuntEnemy(CParabot *pb, PB_Percept*item) {
	...
	if (item->hasBeenSpotted() || !(item->isTrackable())) {
		return 0;
	}
...
float weightTakeCover(CParabot *pb, PB_Percept*item) {
	...
	if (
		(worldTime() - item->lastSeenTime) > 1 && // not seen for too long
		!item->hasBeenSpotted() && // we haven't seen anything yet
		item->distance < 150 && // too close
		item->rating > 0 // we have advantage
	) {
		return 0;
	}
	...
	if (item->isHurtingBot() || item->isAlert()) {
		weight += 3;
	}
	...
	if (item->hasFocus()) {
		weight += 1; // stick to current enemy
	}
...
float weightCloseCombat(CParabot *pb, PB_Percept*item) {
	...
	if ((worldTime() - item->lastSeenTime) > 5) {
		return 0; // not seen for too long
	}
	//if (!pb->combat.weapon.bestWeaponUsable()) return 0;
	if (!item->hasBeenSpotted()) {
		return 0; // we haven't seen anything yet
	}
	if (item->distance < 100) {
		return 15 + (item->rating/5); // va banque
	}
	if (item->rating < -3) {
		return 0; // too bad, better don't try that...
	}
...
//	if (!camPlayer) startBotCam(INDEXENT(1));
	...
	if (item->distance > 500) {
		// too far to approach, just fire from current position
		...
		switch (mod_id) {
			...
			case GEARBOX_DLL:
			case VALVE_DLL:
				pb->combat.weapon.setPreferredWeapon(VALVE_WEAPON_SHOTGUN, 2);
				break;
			case HOLYWARS_DLL:
				pb->combat.weapon.setPreferredWeapon(HW_WEAPON_DOUBLESHOTGUN, 1);
				break;
			case DMC_DLL:
				pb->combat.weapon.setPreferredWeapon(DMC_WEAPON_SUPERSHOTGUN, 1);
				break;
		}
		if (item->distance > 50) {
...
float weightSilentAttack(CParabot *pb, PB_Percept*item) {
	...
	if (pb->senses.underFire() || !pb->combat.weapon.bestWeaponUsable()) {
		return 0;
	}
	...
	if (item->isVisible()) {
		...
		if (item->hasJustDisappeared()) {
			...
			if ((dot > 0.7) && (item->distance > 250)) {
				...
				if ((bestScore > 0) && canShootAt(pb->ent, prePos)) {
					...
					if (
						(mod_id == VALVE_DLL || mod_id == AG_DLL || mod_id == HUNGER_DLL || mod_id == GEARBOX_DLL) &&
						bestWeapon == VALVE_WEAPON_RPG
					) {
				...
				debugMsg("Bot turned round or too near!\n");
			}
		...
		if (item->isUnderPreemptiveFire()) {
			...
			if (item->canBeAttackedBest()) {
				...
				if (fired) {
					pb->preemptiveFire = false; // shoot only once
				}
...
float weightShootAtEnemy(CParabot *pb, PB_Percept*item) {
	...
	if (!item->isTrackable()) { // try to look at it:
		if (item->isHurtingBot()) {
			weight = 5; // hidden attacker -> dangerous!
		} else {
			weight = weightReactToUnidentified(pb, item);
		}
	} else { // we see or saw this one:
		if (item->isHurtingBot() || item->isAimingAtBot()) {
			weight = 5; // shoot if he attacks us
		} else if (item->rating < -3 && !item->isFacingBot()) {
			weight = 0; // too bad, don't evoke attention
		} else {
			weight = 2; // default
		}
		
		if (!item->isVisible() && !item->isUnderPreemptiveFire()) {
			weight /= 2; // still look in direction if no other enemy around
		}
		if (item->hasHighPriority()) {
			weight += 5;
		}
...
void goalArmBestWeapon(CParabot *pb, PB_Percept*item) {
	...
	if (item && item->isTrackable()) {
		...
		Vector botPos = pb->botPos();
		if (item->predictedAppearance(botPos).z > (botPos.z + 20)) {
			botFlags |= WF_ENEMY_ABOVE;
		} else if (item->predictedAppearance(botPos).z < (botPos.z - 80)) {
			botFlags |= WF_ENEMY_ABOVE;
		}
	...
	if (!bestArmed) {
		pb->combat.nextWeaponCheck = worldTime() + CHANGE_WEAPON_DELAY;
	}
	if (bestArmed && item) {
		item->flags |= PI_BEST_ARMED;
	}
...
float weightArmBestWeapon(CParabot *pb, PB_Percept*item) {
	...
	// aim a while before decision
	if (item && item->isTrackable() && (worldTime()-item->firstDetection > 0.5)) {
		if (
			((worldTime()+CHECK_WEAPON_COMBAT) < pb->combat.nextWeaponCheck) ||
			(worldTime() > pb->combat.nextWeaponCheck)
		) {
			weight = 5;
		}
		if (item->hasFocus()) {
	...
	} else if (worldTime() > pb->combat.nextWeaponCheck) {
		weight = 5;
	}
