...
bool PB_Roaming::bigGapAt(Vector pos) {
	...
	return ((distXp + distXn) > 32) && ((distYp + distYn) > 32);
}
...
void PB_Roaming::checkJump(vec3_t origin, vec3_t dir, checkWayRes *res) {
	...
	if (
		// gap goes deeper than target
		((((floor.z + 36 + MAX_ZREACH) < target.z) && ((floor.z + 36 + 16) < origin.z)) ||
...
void PB_Roaming::checkFront (float sideOfs, Vector tAngle, checkWayRes *res) {
	...
	bool needJump = false, jumpBlocked = false, needDuck = false, duckBlocked = false;
	...
	if (jumpBlocked && duckBlocked) {
		res->blocked = true;
		if (!needJump) {
			planeAngle = UTIL_VecToAngles (tr.vecPlaneNormal); // not calculated yet...
		}
		res->wallAngle = planeAngle;
		return; // no gap check if blocked
	}
	...
	// needjump ||
	if (needDuck || jumpBlocked || duckBlocked) {
		return; // not free
	}
	
	if (!action->jumping()) {
		checkJump (pev->v.origin + vSide, vDir, res);
	}
...
void PB_Roaming::checkSide (int side, float frontOfs, checkWayRes *res) {
	...
	if (side == LEFT) {
		vDir = -MAX_WALL_DISTANCE*gpGlobals->v_right;
	} else if (side == RIGHT) {
		vDir = MAX_WALL_DISTANCE*gpGlobals->v_right;
	} else {
		return;
	...
	if (tr.flFraction < 1.0) {
		...
		if (tr.flFraction > MAX_TOUCH) {
			res->tooFar	= true;
		}
		if (tr.flFraction < MIN_TOUCH) {
			res->tooClose = true;
		}
		...
		if (UTIL_PointContents(start + vDir + Vector(0, 0, -40)) == CONTENTS_EMPTY) {
			...
			UTIL_TraceLine(
				start + vDir + Vector(0, 0, -40), start + Vector(0, 0, -40), ignore_monsters, ENT(pev), &tr
			);
			//if (tr.flFraction < 1.0) {
...
void PB_Roaming::checkWay(const Vector &targetPos) {
	...
	if (followLeft) {
		...
		if (!checkIfPassage) { // following the wall
			...
			if (moved.Length()>=passageDistance) {
				if (side.onTouch) { // that's the wall we were looking for 
					...
					if (debugWay) {
						debugMsg("Alligned to new wall.\n");
					}
				} else { // very short wall!
					followLeft = false; // let him free
					if (debugWay) {
						debugMsg("Could not find wall!\n");
					}
				}
				checkIfPassage = false;
				...
				if (passageTries > 8) {
					...
					if (passageTries > 20) {
						...
						followLeft = false; // let him free
						if (debugWay) {
							debugMsg("Could not reach it! dist=%.f, gone=%.f.\n",passageDistance,moved.Length());
						}
					}
				}
			}
		...
		if (right.blocked) { // in any case check for wall in front
			...
			if (debugWay) {
				debugMsg(" Blocked, turning right.\n");
			}
		}
	} else if (followRight) {
		...
		if (!checkIfPassage) { // following the wall
			if (!side.onTouch && (passedEdge < gpGlobals->time)) { // it has disappeared...
				...
				right.shouldJump = false;
				if (
					(std::abs(UTIL_AngleDiff(action->moveAngleYaw(),targetAngle.y))<=90) &&
					(std::abs(UTIL_AngleDiff(action->moveAngleYaw()-90,targetAngle.y))<=90)
				) {
					followRight = false;
					if (debugWay) {
						debugMsg("Freed from right wall.\n");
					}
				} else {
					...
					passageTries = 0;
					if (debugWay) {
						debugMsg("Searching passage at the right... ");
					}
				}
			} else { // everything fine
				passageDistance = side.wallDistance + 24; // if wall disappears next frame
				if (side.tooClose || right.blocked) {
					action->add(BOT_STRAFE_LEFT);
				}
				else if (side.tooFar) {
					action->add(BOT_STRAFE_RIGHT);
		...
		if (left.blocked && right.blocked) { // suddenly a wall
			...
			if (left.wallAngle.y == right.wallAngle.y) { // in front of plain wall
				...
				if (std::abs(adtw)>135) { // target straight behind
					...
					if (turn == NO_DIR) {
						if (debugWay) {
							debugMsg("Blocked, no exit found: ");
						}
						turn = RANDOM_LONG (LEFT,RIGHT);
					} else if (debugWay) {
						debugMsg("Blocked, found exit: ");
					}
					...
					if (debugWay) {
						debugMsg("Blocked, heading for target: ");
					}
				}
			} else { // in corner
				turn = RANDOM_LONG (LEFT,RIGHT);
			...
			if (turn == LEFT) {
				if (debugWay) {
					debugMsg("Turning left, following wall.\n");
				}
				action->setMoveAngleYaw (left.wallAngle.y - 90); // follow wall to left
				followRight = true; // wall should be at the right
			} else if (turn == RIGHT) {
				if (debugWay) {
					debugMsg("Turning right, following wall.\n");
				}
				action->setMoveAngleYaw (right.wallAngle.y + 90); // follow wall to right
			...
			if (left.blocked) {
				...
				if (strafeRightCount > 3) {
					if (debugWay) {
						debugMsg("Strafing too long, mode set to followLeft\n");
					}
					followLeft = true;
					strafeRightCount = 0;
				}
			...
			if (right.blocked) {
				...
				if (strafeLeftCount > 3) {
					if (debugWay) {
						debugMsg("Strafing too long, mode set to followRight\n");
					}
					followRight = true;
					strafeLeftCount = 0;
				}
	...
	if ((left.shouldDuck || right.shouldDuck)) {
		action->add(BOT_DUCK); // ...or duck if necessary
	}
	...
	targetVisible = tr.flFraction == 1.0 || tr.pHit == pev;
	
	if (targetVisible) { // special behaviour if target is visible:
		if (
			(targetDistance < 70) || // - too close to target, ignore walls
			(
				(followLeft||followRight) && sideValid &&
				(std::abs(UTIL_AngleDiff(side.wallAngle.y,targetAngle.y)) <= 90)
			)
		) {	
			//if (debugWay) 
...
bool PB_Roaming::targetNotReachable() {
	...
	if (xyDist < 40 && xyDist > lastXyDist) {
		return true;
	}
