...
#define IGNORE_DAMAGE (                                              \
	DMG_CRUSH | DMG_FREEZE | DMG_FALL | DMG_SHOCK | DMG_DROWN |      \
...
*/
	...
	if (!found) { // now we'll have to guess
		...
		for (int i = 1; i <= gpGlobals->maxClients; i++) {
			pPlayer = UTIL_PlayerByIndex(i);
			if (
				!pPlayer || pPlayer->pev == &(ent->v) || !isAlive(ENT(pPlayer->pev)) ||
...
/*			int usedWeapon = clientWeapon[i];
			...
			if (aim < 0.95 || !canShootAt(pPlayer->edict(), botPos())) {
				continue;
			}
...
bool CParabot::positionReached(Vector pos) {
	Vector v = botPos() - pos;
	return v.Length() < PB_REACH_DISTANCE;
}
...
void CParabot::pathCheckWay() {
	...
	if (actualPath) {
		...
		if (actualPath->waitForPlatform()) {
			...
			if (
				(platPos - lastPos).Length() < 50  &&
				  ent->v.groundentity!=0 && 
				  ent->v.groundentity->v.size == Vector(2,2,2)
			) {// worldSpawn
...
void CParabot::checkForTripmines() {
	...
	if (tAll) {
		if (mine_vecDir.z == 0) { 
			...
			debugMsg("Tripmine vertical, can't evade\n");
		}
	}
	...
	if (actualPath->finished(ent)) {
		...
		if (mustShootObject) {
			switch (mod_id) {
				...
				case GEARBOX_DLL:
				case VALVE_DLL:
					combat.weapon.setPreferredWeapon(VALVE_WEAPON_GLOCK, 1);
					break;
				case HOLYWARS_DLL:
					combat.weapon.setPreferredWeapon(HW_WEAPON_DOUBLESHOTGUN, 1);
					break;
				case DMC_DLL:
					combat.weapon.setPreferredWeapon(DMC_WEAPON_QUAKEGUN, 1);
					break;
			}
		...
		if (!actualPath) {
			return; // maybe tripmine canceled path
		}
...
void CParabot::followActualRoute() {
	...
	if (!isUnderwater(ent)) {
		...
		float xyDist = xyDir.Length();
		float xyzDist = xyzDir.Length();
		if (xyDist < 30 && xyzDist > 50) {
			action.setSpeed(8 * xyDist);
		} else {
			action.setMaxSpeed();
		}
	} else {
	...
	if (action.gotStuck() || (worldTime() > cellTimeOut)) {
		...
		if ((lastJumpPos - botPos()).Length() > 50) {
			...
			// bisherigen traffic auf Teilstrecke auswerten, falls <3 Verbindung l�schen
			if (map.cell(botCell).getTraffic(targetCell) < 3) {
				if (map.cell(botCell).delNeighbour(targetCell)) {
					...
					Vector c2 = map.cell(targetCell).pos();
					debugBeam(c1, c2, 250, 0); // rot vom Vorg�nger
					debugBeam(c0, c2, 250, 1); // gr�n vom bot aus
					debugMarker(c2, 250);
...
void CParabot::executeGoals() {
	...
	for (int i = 0; i < MAX_GOALS; i++) {
		...
		if (goalFunction) {
			...
			if (trigger!=0) {
				assert(trigger->pClass > 0 && trigger->pClass <= MAX_PERCEPTION);
			}
