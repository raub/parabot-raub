...
static tWeaponRec valveWeapon[MAX_VALVE_WEAPONS] = {
	...
	{ "weapon_9mmhandgun", 200, 0.052, 0.5, false, WV_NORMAL, WV_NORMAL, WF_NORMAL, WF_NORMAL, 0.0, "glock" },
	{ "weapon_357", 600, 0.017, 0.5, false, WV_LOUD, WV_NONE, WF_NORMAL, WF_NONE, 0.0, "python" },
	{ "weapon_9mmAR", 200, 0.104, 0.4, true, WV_NORMAL, WV_NORMAL, WF_NORMAL, WF_BRIGHT, 0.0, "mp5" },
	{ "", 0, 0.1, 0.0, false, WV_NONE, WV_NONE, WF_NONE, WF_NONE, 0.0, "" }, // doesn't exist!
...
static tWeaponRec gearboxWeapon[MAX_GEARBOX_WEAPONS] = {
	...
	{ "weapon_9mmhandgun", 200, 0.052, 0.5, false, WV_NORMAL, WV_NORMAL, WF_NORMAL, WF_NORMAL, 0.0, "glock" },
	{ "weapon_357", 600, 0.017, 0.5, false, WV_LOUD, WV_NONE, WF_BRIGHT, WF_NONE, 0.0, "python" },
	{ "weapon_9mmAR", 200, 0.104, 0.5, true, WV_NORMAL, WV_NORMAL, WF_NORMAL, WF_BRIGHT, 0.0, "mp5" },
	{ "", 0, 0.0, 0.0, false, WV_NONE, WV_NONE, WF_NONE, WF_NONE, 0.0, "" }, // doesn't exist!
	{ "weapon_crossbow", 600, 0.017, 0.5, false, WV_QUIET, WV_NONE, WF_NONE, WF_NONE, 0.0, "crossbow" },
	{ "weapon_shotgun", 50, 0.173, 0.5, false, WV_LOUD, WV_LOUD, WF_NORMAL, WF_NORMAL, 0.0, "shotgun" },
...
static tWeaponRec hungerWeapon[MAX_HUNGER_WEAPONS] = {
	...
	{ "weapon_9mmhandgun", 200, 0.052, 0.5, false, WV_NORMAL, WV_NORMAL, WF_NORMAL, WF_NORMAL, 0.0, "beretta" },
	{ "weapon_357", 600, 0.017, 0.5, false, WV_LOUD, WV_NONE, WF_NORMAL, WF_NONE, 0.0, "python" },
	{ "weapon_9mmAR", 200, 0.104, 0.4, true, WV_NORMAL, WV_NORMAL, WF_NORMAL, WF_BRIGHT, 0.0, "Thompson" },
...
float PB_Weapon::valveWeaponScore(float distance, float hitProb, int flags, bool checkAmmo) {
	...
	case VALVE_WEAPON_SHOTGUN:
		if (flags & (WF_UNDERWATER | WF_NEED_GRENADE)) {
			break;
		}
		...
		if (hitProb < 0.5) {
			score *= (hitProb + 0.5); // punish slow reload
		}
...
float PB_Weapon::getScore(float distance, float hitProb, int flags, bool checkAmmo) {
	...
	case AG_DLL:
	case VALVE_DLL:
		return valveWeaponScore(distance, hitProb, flags, checkAmmo);
	case HOLYWARS_DLL:
		return hwWeaponScore(distance, hitProb, flags, checkAmmo);
	case DMC_DLL:
		return dmcWeaponScore(distance, hitProb, flags, checkAmmo);
	case CSTRIKE_DLL:
		return csWeaponScore(distance, hitProb, flags, checkAmmo);
	case TFC_DLL:
		return tfcWeaponScore(distance, hitProb, flags, checkAmmo);
	case HUNGER_DLL:
		return hungerWeaponScore(distance, hitProb, flags, checkAmmo);
	case GEARBOX_DLL:
		return gearboxWeaponScore(distance, hitProb, flags, checkAmmo);
	}
			...
			if (bestMode[currentWeapon] == 1) {
				...
				if (mod_id == VALVE_DLL || mod_id == AG_DLL || mod_id == HUNGER_DLL || mod_id == GEARBOX_DLL) {
					if (currentWeapon == VALVE_WEAPON_GAUSS) {
						...
						currentWeapon == VALVE_WEAPON_CROSSBOW ||
