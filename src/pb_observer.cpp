...
int PB_Observer::registerPlayer(edict_t *player) {
	...
	while (	obs[i].player!=0 && i < MAX_OBS) {
		i++;
	}
...
void PB_Observer::registerClients() {
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++) {
		player = INDEXENT(i);
		if (!playerExists(player) || !isAlive(player)) {
			continue;
		}
...
int PB_Observer::checkGround(int oId, edict_t **plat) {
	*plat = 0;
	...
	if (obs[oId].player->pev->movetype == MOVETYPE_FLY) {
		flags |= WP_ON_LADDER;
	}
		...
		if (strcmp(groundName, "worldspawn") != 0) {
			...
			if (
				...
				if (sId < 0) {
					...
					if (obs[oId].platform == 0) {
						flags |= WP_AT_PLATFORM_START;
					}
...
//						observedPath[oId].addPlatformInfo(sId, ground->v.absmin);
			...
			if (obs[oId].platform) {
				...
				int sId = obs[oId].lastPlatId;
				if (sId >= 0) { // have we got the right one?
					if (getNavpoint(sId).entity() != obs[oId].platform) {
						sId = -1;
					}
				}
				if (sId < 0) {
...
bool PB_Observer::shouldObservePlayer(int oId) {
	...
	if (obs[oId].active) { // check if observation has to stop...
		assert (obs[oId].player != 0);
		if ((obs[oId].player->pev->health < 1) || (obs[oId].player->pev->solid == SOLID_NOT)) {
			// ...yes -> mark inactive
...
void PB_Observer::newNavpointReached(int oId, Vector &pos, PB_Navpoint *endNav) {
	...
	if (endNav->type() == NAV_INFO_TELEPORT_DEST && (obs[oId].lastFramePos - pos).Length() > 100) {
	...
	if (!FBitSet(obs[oId].player->pev->flags, FL_FAKECLIENT)) {
		...
		if (startIndex != -1) { // beginning of this path was recorded
			...
			while (currentIndex != obs[oId].leadWaypoint && dbgCnt++ < 1000) {
				...
				newPath.addWaypoint(
					waypoint[oId][currentIndex].data.pos,
				...
				if (waypoint[oId][currentIndex].isOnPlatform()) {
					...
					newPath.addPlatformInfo(
						platInfo[oId][currentIndex].data.navId,
			...
			if (pathMode & PATH_NEED_LONGJUMP) {
				debugMsg("PATH NEEDS LONGJUMP!\n");
			}
			if (pathMode & PATH_CAUSES_DAMAGE) {
				debugMsg("PATH CAUSES DAMAGE!\n");
			}
...
void PB_Observer::checkForJump(int oId, Vector &pos) {
	...
	if (!obs[oId].jumpPressed && (obs[oId].player->pev->button & IN_JUMP)) {
		...
		if (
			(obs[oId].player->pev->button & IN_DUCK) &&
...
void PB_Observer::checkForUse(int oId, Vector &pos) {
	...
	if (!obs[oId].usePressed && (obs[oId].player->pev->button & IN_USE)) {
		...
		int navType = -1;
		if (obs[oId].lastReachedNav) {
			navType = obs[oId].lastReachedNav->type();
		}
...
void PB_Observer::checkForMove(int oId, Vector &pos) {
	...
	bool sharpTurn = (
		(std::abs(UTIL_AngleDiff(obs[oId].lastWpYaw, obs[oId].player->pev->v_angle.y)) > 90) &&
	...
	if (((pos - obs[oId].lastWpPos).Length() > MIN_WAYPOINT_DIST) || sharpTurn) {
		...
		//debugMsg("x");
		if (
			FBitSet(obs[oId].player->pev->flags, FL_ONGROUND) || // on ground
			(obs[oId].player->pev->waterlevel > 0) || // or in water
			(obs[oId].player->pev->movetype == MOVETYPE_FLY)
		) {
			// or on ladder
			//int flags = checkGround(oId);
	...
	if (obs[oId].player->pev->frags != obs[oId].frags) {
		if (obs[oId].player->pev->frags > obs[oId].frags) {
			...
			if ((worldTime() - obs[oId].lastWpTime) > MIN_CAMP_TIME) {
				...
				if (
					(nearestCamp && ((nearestCamp->pos()-pos).Length() < 128)) ||
...
void PB_Observer::checkForTripmines(int oId, Vector &pos) {
	...
	if (
		!((obs[oId].player->pev->button & IN_ATTACK) && (playerWeapon == VALVE_WEAPON_TRIPMINE)) &&
		!(
...
void PB_Observer::checkForButtonShot(int oId, Vector &pos) {
	...
	if (!(obs[oId].player->pev->button & IN_ATTACK) || !(playerWeapon != DMC_WEAPON_CROWBAR)) {
		return;
	}
	...
	if (tr.flFraction < 1.0 && tr.pHit != 0) {
		...
		if (strcmp(hitClass, "func_button") == 0 && tr.pHit->v.health > 0) {
...
void PB_Observer::checkPlayerHealth(int oId) {
	...
	if (obs[oId].player->pev->dmg_take > 0) {
		if ((worldTime()-obs[oId].fallTime) < 0.3) {
			...
			(obs[oId].player->pev->watertype == CONTENT_LAVA) ||
...
void PB_Observer::updateCellInfo(int i) {
	...
	if (
		obsCell == NO_CELL_FOUND &&
		UTIL_PointContents(obsPos) != CONTENTS_SOLID && // Bugfix!!!
		(
	...
	if (obs[i].lastCell == NO_CELL_FOUND) {
		obs[i].lastCell = obsCell;
	}
	...
	if (obs[i].currentCell != NO_CELL_FOUND) {
		...
		if (
			(obs[i].lastCell != obs[i].currentCell && distToCurrentCell < 25) || 
			...
			if (bNr >= 0) {
				...
				float fd1 = (
					map.cell(obs[i].currentCell).focus.cellsForDir(Vector(1,0,0)) +
					3*map.cell(obs[i].currentCell).kills.forDir(Vector(1,0,0))
				) * (1.5 + moveDir.x);
				float fd2 = (
					map.cell(obs[i].currentCell).focus.cellsForDir(Vector(0,1,0)) +
			...
			if (visualizeCellConnections) {
				// draw traffic beam
				debugBeam(
					map.cell(obs[i].currentCell).pos() + Vector(0,0,-30),
	...
	for (int i = 0; i < MAX_OBS; i++) {
		if (shouldObservePlayer(i)) { // still observing this one
			...
			if (nav->reached(obs[i].player->edict())) {
				...
				if (
					(nav!=obs[i].lastReachedNav) &&
					(nav->entity() != obs[i].player->pev->groundentity)
				) {
					// take care with plats,
...
void PB_Observer::addWaypoint(int oId, Vector pos, int action, int col) {
	...
	if (obs[oId].leadWaypoint == MAX_WPTS) {
		obs[oId].leadWaypoint = 0;
	}
...
//		debugMsg("Reached FWP %i\n", currentWaypoint[botId]);
		currentWaypoint[botId]++;
		if (currentWaypoint[botId]==MAX_WPTS) {
			currentWaypoint[botId] = 0;
		}
	}
	/*	int nr = partner[botId];
...
bool PB_Observer::shouldFollow(int botId, edict_t *bot) {
	...
	assert(bot != 0);
	if (
		(getNextWaypoint(botId).action() == BOT_JUMP) ||
		!FBitSet(bot->v.flags, FL_ONGROUND)
	) {
		return true;
	}
...
bool PB_Observer::partnerValid(int botId) {
	int nr = partner[botId];
	return obs[nr].active && (obs[nr].player != 0);
}
