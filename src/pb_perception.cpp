...
static float perceptionDist[MAX_PERCEPTION + 1] = {
	0,
	MAX_DIST_VP, // player
	MAX_DIST_VP, // friend
	MAX_DIST_VP, // foe
...
Vector PB_Percept::predictedAppearance(const Vector &botPos) {
	...
	if (predAppearance == UNKNOWN_POS) {
		...
		if (start == NO_CELL_FOUND || target == NO_CELL_FOUND) {
...
bool PB_Percept::isAimingAtBot() {
	...
	if (worldTime() >= (firstDetection + recTime)) {
		return (orientation > 0.95);
	}
...
void PB_Perception::addAttack(edict_t *inflictor, int dmg) {
	...
	tactileDetections.push_back(PB_Percept(
		sensitivity, inflictor, PI_TACTILE, PI_DAMAGE, static_cast<float>(dmg)
	));
...
bool PB_Perception::classify(PB_Percept &perc) {
	...
	if (
		(mod_id == VALVE_DLL || mod_id == HUNGER_DLL || mod_id == GEARBOX_DLL || mod_id == DMC_DLL) &&
		!valveTeamPlayMode
	) {
		...
		if (worldTime() >= (perc.firstDetection + recTime)) {
			if (UTIL_GetTeam(perc.entity) != UTIL_GetTeam(botEnt)) {
				perc.pClass = PI_FOE;
			} else {
...
bool PB_Perception::isNewPerception(tPerceptionList &oldList, PB_Percept &perc) {
	...
	if (match != oldList.end()) { // match found?
		...
		if (match->flags & PI_FOCUS1) {
			perc.flags |= PI_FOCUS2; // remember focus once
		}
...
bool PB_Perception::addIfVisible(Vector pos, edict_t *ent, int pClass) {
	...
	if (dist > (sensitivity*perceptionDist[pClass])) {
		return false;
	}
...
void PB_Perception::checkDamageFor(PB_Percept &player) {
	...
	while (cdi != detections[cdet].end()) {
		if (
			...
			if (cdi->entity == 0) {
				cdi->entity = player.entity; // assume player is responsible
			}
		}
		cdi++;
	}
...
void PB_Perception::checkInflictorFor(PB_Percept &dmg) {
	...
	while (cdi != detections[cdet].end()) {
		if (
			(cdi->pClass < PI_HOSTAGE && cdi->pClass != PI_FRIEND) && 
...
void PB_Perception::collectData() {
	...
	while((ent = UTIL_FindEntityInSphere(ent, botEnt->v.origin, sensitivity * MAX_DIST_VP))) {
		...
		if (strcmp(pClassname, "player") == 0) {
			// check if valid
			if (ent->pev == &(botEnt->v) || !isAlive(ENT(ent->pev)) || ent->pev->solid == SOLID_NOT) {
				// skip self
				// skip player if not alive
				// skip player if observer
				continue;
			}
			
			detected = addIfVisible(ent->edict(), PI_PLAYER);
			if (!detected) {
				detected = addIfVisible(ent->pev->absmin, ent->edict(), PI_PLAYER);
			}
			if (!detected) {
				detected = addIfVisible(ent->pev->absmax, ent->edict(), PI_PLAYER);
			}
			...
			(mod_id != DMC_DLL && strcmp(pClassname, "weaponbox") == 0) ||
	...
	while (cdi != detections[cdet].end()) {
		...
		if (isNewPerception(detections[odet], *cdi)) {
			checkDamageFor(*cdi);
		}
	...
	while (odi != detections[odet].end()) {
		if ((worldTime()-odi->lastDetection) < 5) {
			...
			if ((odi->pClass == PI_HALO || odi->pClass == PI_WEAPONBOX) && (odi->distance < 50)) {
				odi++;
				continue;
			} // no items that have already been picked up
	...
	while (cdi != detections[cdet].end()) {
		if (cdi->pClass == PI_FOE) {
			numEnemies++;
		} else if (cdi->pClass == PI_PLAYER) {
			numUnidentified++;
		}
		cdi++;
	}
...
bool PB_Perception::underFire() {
	...
	if (worldTime() < (lastCall + 0.1)) {
		return lastReturn; // 10 updates/sec. ok
	}
