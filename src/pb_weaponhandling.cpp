...
bool PB_WeaponHandling::available(int wId) {
	if (pbConfig.onRestrictedWeaponMode()) {
		...
		case HOLYWARS_DLL:
			if (
				wId == HW_WEAPON_ROCKETLAUNCHER ||
				wId == HW_WEAPON_RAILGUN
			) {
				return false;
			}
			break;
		...
		case GEARBOX_DLL:
			if (
				...
				wId == GEARBOX_WEAPON_SNIPERRIFLE
			) {
				return false;
			}
			break;
