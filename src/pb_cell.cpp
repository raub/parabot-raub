...
PB_Cell::PB_Cell(edict_t *pEdict) {
	...
	if (
		np &&
		(np->pos() - data.position).Length() <= CELL_SIZE  &&
...
bool PB_Cell::addTraffic(short nbId, float nbWeight) {
	...
	for (int i = 0; i < MAX_NBS; i++) {
		if (data.neighbour[i] == nbId) {
			if (data.traffic[i] < 32767) {
				data.traffic[i]++; // take care with short overflow
			}
...
float PB_Cell::getEnvDamage() {
	...
	return (data.envDamage / ((float)data.visits));
}
...
bool PB_Cell::isSuitableRoamingTarget(edict_t *traveller) {
	...
	if (worldTime() >= mapGraph[data.navpoint].first.nextVisit(traveller)) {
		return true;
	}
