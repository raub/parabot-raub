	...
	for (int i = 0; i < numberOfNavpoints(); i++) {
		if ((i != ignoreId) && (worldTime() >= graph[i].first.nextVisit(traveller))) {
			...
			if (graph[i].second.size() > 0) {
				dist /= 2; // prefer linked navpoints
			}
...
void PB_MapGraph::Dijkstra(
	std::vector<float>& dist, std::vector<int>& path, int start, int searchMode
) {
	// init distances
...
int PB_MapGraph::DijkstraToWish(
			...
			if (p.valid(searchMode)) {
				...
				if (dist[neighbor] > (dist[actualVertex] + d)) {
					...
					if (graph[neighbor].first.nextVisit(traveller) < worldTime()) {
						...
						score[neighbor] = (
							score[actualVertex] +
							(needs.desireFor(graph[neighbor].first.type()) / navWeight)
						);
					}
				}
			}
...
bool PB_MapGraph::save(char *mapname) {
	...
	for (i = 0; i < graph.size(); i++) {
		...
		while (adj != graph[i].second.end()) {
			...
			if (notUsed > maxNotUsed) {
				adj->second.markForDelete();
			}
			adj++;
		}
	}
