...
float PB_Combat::getRating(PB_Percept &perceipt) {
	...
	if (isInvulnerable(perceipt.entity)) {
		return -5; // don't mess with this one!
	}
	...
	if (perceipt.isFacingBot()) {
		...
		if (enemyWeapon != botWeapon) { // different weapons: calculate...
			...
			// enemy variables
			int enemyFlags = 0;
			if (enemy->v.waterlevel == 3) {
				enemyFlags |= WF_UNDERWATER;
			}
			float enemyHitProb = perceipt.targetAccuracy();
			adv = (
				weapon.getWeaponScore(botWeapon, enemyDist, botHitProb, botFlags, true) -
...
bool PB_Combat::shootAtEnemy(edict_t *enemy, float accuracy) {
	...
	case DMC_DLL:
		if (weapon.currentWeapon()==DMC_WEAPON_ROCKETLAUNCHER) {
			...
			weapon.currentWeapon()==DMC_WEAPON_NAILGUN ||
...
// decides which reaction is most appropiate and calls either engage() or retreat() {
	...
	if (enemyDist < 1000) {
		if (enemyWeapon != botWeapon) { // different weapons: calculate...
			...
			float nearAdv = (
				weapon.getWeaponScore(botWeapon, enemyDist - 25, botHitProb, botFlags, true) -
			...
			if ((farAdv > currentAdv) && (farAdv > nearAdv)) {
				gainDistance = true;
			}
			if ((nearAdv > currentAdv) && (nearAdv > farAdv)) {
				closeUp = true;
			}
	...
	switch (mod_id) {
		...
		case GEARBOX_DLL:
		case VALVE_DLL:
			closeDistanceWeapon = VALVE_WEAPON_CROWBAR;
			break;
		case HOLYWARS_DLL:
			closeDistanceWeapon = HW_WEAPON_JACKHAMMER;
			break;
		case DMC_DLL:
			closeDistanceWeapon = DMC_WEAPON_CROWBAR;
			break;
	}
	
	// don't evade with this
	if (perceipt.isVisible() && perceipt.isAimingAtBot() && (botWeapon!=closeDistanceWeapon)) {
