...
static char navpointClasses[MAX_NAV_TYPES][32] = {
	"unknown",
	"weapon_crossbow",
	"weapon_crowbar",
	"weapon_egon",
...
Vector PB_Navpoint::pos(edict_t *playerEnt) {
	assert(playerEnt!=0);
	if (playerEnt->v.movetype != MOVETYPE_FLY) {
		return data.pos;
	} else {
...
bool PB_Navpoint::isTriggerFor(PB_Navpoint &wp) {
	...
	if (type()==NAV_S_BUTTON_SHOT) {
		triggerEnt = getNavpoint(data.special).entity();
	}
	
	if (!targetEnt || !triggerEnt) {
		return false;
	}
	
	if (FStringNull(targetEnt->v.targetname) || FStringNull(triggerEnt->v.target)) {
		return false;
	}
	
	if (UTIL_ButtonTriggers(triggerEnt, targetEnt)) {
		return true;
	}
...
bool PB_Navpoint::isTriggered() {
	...
	if (mod_id != DMC_DLL) {
		return toggleClass->m_toggle_state != normalState;
	}
	
	// for DMC:
	if (type()==NAV_F_DOOR) {
		if (ent->v.spawnflags & SF_DOOR_START_OPEN) {
			return toggleClass->m_toggle_state != TS_AT_BOTTOM;
		} else {
			return toggleClass->m_toggle_state != TS_AT_TOP;
		}
	} else {
		return toggleClass->m_toggle_state != TS_AT_TOP;
	}
...
void PB_Navpoint::reportVisit(edict_t *player, float time) {
	...
	case VALVE_DLL:
		if (type() < NAV_F_BUTTON) {
			...
			nextVisitAt = lastVisitedAt + 10; // no respawn
		}
		break;
	case DMC_DLL:
		if (type() == NAV_DMCI_HEALTH_LARGE) {
			...
			(type()>=NAV_DMCI_ARMOR1 && type()<=NAV_DMCI_ARMOR_INV) ||
		...
		break;
	default:
		nextVisitAt = lastVisitedAt + 10; // forced to tour...
		break;
	}
}
...
bool PB_Navpoint::offersHealth() {
	// instead of assert because of bug report
	if (data.type!=NAV_F_HEALTHCHARGER || !ent) {
		return false;
	}
	
	return ent->v.frame == 0;
}


bool PB_Navpoint::offersArmor() {
	// instead of assert because of bug report
	if (data.type!=NAV_F_RECHARGE || !ent) {
		return false;
	}
