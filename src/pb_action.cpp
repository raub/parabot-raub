...
Vector PB_Action::calcViewAngle() {
	...
	if (toAdd.x > maxTurn) {
		toAdd.x = maxTurn;
	} else if (toAdd.x<-maxTurn) {
		toAdd.x = -maxTurn;
	}
	
	if (toAdd.y > maxTurn) {
		toAdd.y = maxTurn;
	} else if (toAdd.y<-maxTurn) {
		toAdd.y = -maxTurn;
	}
...
/*	if (speed > 0 && !notStucking) {
	...
	g_engfuncs.pfnRunPlayerMove(
		ent, moveAngle, speed, strafe, 0, action, 0, static_cast<byte>(msec())
	);
}
