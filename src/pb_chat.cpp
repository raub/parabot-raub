...
void pfnEmitSound(
	edict_t *entity, int channel, const char *sample,
	/*int*/float volume, float attenuation, int fFlags, int pitch
);
// from engine.h

void pfnEmitAmbientSound(
	edict_t *entity, float *pos, const char *samp,
	float vol, float attenuation, int fFlags, int pitch
);
...
bool PB_Chat::free() {
	...
	for (i = 0; i < chatGotKilled.size(); i++) {
		delete chatGotKilled[i].text;
	}
	chatGotKilled.clear();
	
	for (i = 0; i < chatKilledPlayer.size(); i++) {
		delete chatKilledPlayer[i].text;
	}
	chatKilledPlayer.clear();
	
	for (i = 0; i < chatGotWeapon.size(); i++) {
		delete chatGotWeapon[i].text;
	}
	chatGotWeapon.clear();
	
	for (i = 0; i < chatJoin.size(); i++) {
		delete chatJoin[i].text;
	}
	chatJoin.clear();
	
	for (i = 0; i < chatReplyUnknown.size(); i++) {
		delete chatReplyUnknown[i].text;
	}
...
PB_ChatMessage* PB_Chat::getMessageFromList(ChatList &clist, bool forceReply) {
	if (clist.size() > 0) {
		...
		do {
			...
			if (
				worldTime() > (clist[msgNr].time + 600.0) ||
...
const char* PB_Chat::getName(edict_t *player) {
	...
	if (clanTagStart) {
		...
		if ((clanTagEnd - clanTagStart) < 5	&& strlen(fullName) > 5 && remove	> 0) {
...
void PB_Chat::parseMessage(edict_t *speaker, const char *msg) {
	...
	if (pbConfig.onAlwaysRespond() && !FBitSet(speaker->v.flags, FL_FAKECLIENT)) {
		forceAnswer = true;
	}
	...
	if (wordFound) {
		answer = getMessageFromList(*(chatReplies[i]->reply), forceAnswer);
	} else {
		answer = getMessageFromList(chatReplyUnknown, forceAnswer);
	}
...
void PB_Chat::registerGotWeapon(edict_t *finder, const char *wpnName) {
	if (!pbConfig.usingChat() || !FBitSet(finder->v.flags, FL_FAKECLIENT)) {
		return;
	}
...
void PB_Chat::registerJoin(edict_t *joiner) {
	if (!pbConfig.usingChat() || !FBitSet(joiner->v.flags, FL_FAKECLIENT)) {
		return;
	}
	
	if (worldTime() < 30.0) {
		return; // don't greet at mapstart (just respawn, no new connect)
	}
